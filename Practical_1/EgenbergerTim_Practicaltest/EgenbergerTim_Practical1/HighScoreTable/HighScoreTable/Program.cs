...
namespace HighScoreTable
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (badinput)
            {
                Console.Write("\n\nPlease enter the number of High Scores: ");

                if ((int.TryParse(Console.ReadLine(), out  scores)) && scores > 0)
                {
                    badinput = false;
                }
                else
            ...
            Console.Write("\n\nPlease enter the " + scores + " High Scores.");

            for (int i = 0; i < HighScores.Length; i++)
            {
                Console.Write("\n\nScore " + (i + 1) + ": ");
                string entry = Console.ReadLine();
                HighScores[i] = int.Parse(entry);
            }
